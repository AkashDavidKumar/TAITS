  ...
  return (
    <nav id="navbar" className={isScrolled ? 'scrolled' : ''}>
      ...
      <div className={`nav-links ${isMobileMenuOpen ? 'active' : ''}`}>
        ...
        {/* Mobile only links */}
        <a href="#" className="mobile-only-link" onClick={(e) => { e.preventDefault(); openSchedule(); closeMobileMenu(); }}>Schedule Meeting</a>
        <a href="#" className="mobile-only-link" onClick={(e) => { e.preventDefault(); openContact(); closeMobileMenu(); }}>Contact Us</a>
      </div>
      ...
      <div className="nav-cta hide-mobile" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
        ...
        <button onClick={openContact} ref={ctaButtonRef} className="btn btn-outline btn-magnetic">Get Started</button>
      </div>

      <div className="mobile-nav-actions" style={{ display: 'none', alignItems: 'center', gap: '1rem' }}>
        <button onClick={toggleTheme} className="theme-toggle-btn" title="Toggle Theme">
          {isLightTheme ? (
            <svg id="moon-icon-mobile" viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
            </svg>
          ) : (
            <svg id="sun-icon-mobile" viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="5" />
              <line x1="12" y1="1" x2="12" y2="3" />
              <line x1="12" y1="21" x2="12" y2="23" />
              <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
